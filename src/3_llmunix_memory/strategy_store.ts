/**
 * Strategy Store — Read/write hierarchical strategy files
 *
 * Strategies are stored as markdown files with YAML-like frontmatter
 * in the strategies/ directory tree:
...
const LEVEL_DIRS: Record<HierarchyLevel, string> = {
...
function strategyFromMarkdown(content: string, filePath: string): Strategy | null {
  try {
    ...
    logger.warn('StrategyStore', `Failed to parse strategy: ${filePath}`, {
...
function parseNegativeConstraints(content: string): NegativeConstraint[] {
...
export class StrategyStore {
  ...
  /**
   * Get all strategies for a given hierarchy level.
   * Reads from both the level directory and _seeds/ directory.
   */
  getStrategiesForLevel(level: HierarchyLevel): Strategy[] {
    if (!this.isAvailable()) return [];

    const levelDir = path.join(this.strategiesDir, LEVEL_DIRS[level]);
  ...
  /**
   * Get all negative constraints, optionally filtered by context keyword.
   */
  ...
  /**
   * Save a strategy to the appropriate level directory.
   */
  saveStrategy(strategy: Strategy): void {
    ...
    const levelDir = path.join(this.strategiesDir, LEVEL_DIRS[strategy.hierarchyLevel]);
    ...
    this.clearCache();
    logger.debug('StrategyStore', `Saved strategy: ${strategy.id}`);
  }

  /**
   * Append a negative constraint to the constraints file.
   */
  saveNegativeConstraint(constraint: NegativeConstraint): void {
    ...
    logger.debug('StrategyStore', `Saved constraint: ${constraint.description}`);
  }

  /**
   * Increment success count and boost confidence for a strategy.
   */
  ...
  /**
   * Decay unused strategies by reducing confidence.
   */
  decayUnusedStrategies(maxAgeDays: number = 30): number {
    ...
    for (const level of Object.values(HierarchyLevel).filter(v => typeof v === 'number') as HierarchyLevel[]) {
      ...
      for (const strat of strategies) {
        // Seeds with no trace evidence don't decay
        if (strat.sourceTraceIds.length === 0) continue;
        // Only decay if confidence is above minimum
  ...
  /**
   * Rebuild the internal index (clear cache, re-scan directories).
   */
  rebuildIndex(): void {
    this.clearCache();
  }
  ...
  /**
   * Get a brief summary of strategies at a given level.
   */
  ...
  /**
   * Read the last dream journal timestamp.
   */
  ...
  /**
   * Find a specific strategy by ID across all levels.
   */
  ...
  private readStrategiesFromDir(dirPath: string): Strategy[] {
